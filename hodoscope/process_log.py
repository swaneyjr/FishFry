...
def analysis(args):
    ...
    chan_all = (chan_a, chan_b, chan_c)
    start_time = min(np.min(chan) for chan in chan_all if np.any(chan))
    end_time = max(np.max(chan) for chan in chan_all if np.any(chan))
