...
if __name__ == '__main__':
    ...
    parser.add_argument('--files', required=True, nargs='+', help='Hodoscope .npz files')
    ...
    parser.add_argument('--channels', required=True, nargs='+', help='One of (ab, ac, bc) for each file')
    parser.add_argument('--path_fit', required=True, help='.npz file with linear fit generated by mc_thresh.py')

    args = parser.parse_args()

    lengths = np.array([len(args.files), 
        len(args.gaps), 
        len(args.labels), 
        len(args.channels)])
    ...
    thresh_min = 256
    thresh_max = 0
    for fname, gap, label, c in zip(args.files, args.gaps, args.labels, args.channels):
        print(fname)
        f = np.load(fname)
